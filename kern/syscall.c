...
static int
sys_pkt_try_send(void* buffer, size_t len){
	user_mem_assert(curenv,buffer,len,PTE_U);
	return e1000_transmit(buffer,len);
}

static int
sys_pkt_try_receive(void* buffer, size_t* len){
	return e1000_receive(buffer,len);
}
